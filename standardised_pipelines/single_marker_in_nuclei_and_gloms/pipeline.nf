    ...
    while IFS= read -r filepath; do
        if [[ -f "\$filepath" ]]; then
            ...
            if [[ -n "\$condition_found" ]]; then
                ...
                echo "Found: condition=\$condition_found, experiment=\$experiment, replicate=\$replicate, date_timestamp=\$date_timestamp,\$channel_idx \$glom_idx \$nuclei_idx file=\$filename" >> \$LOG_FILE
                echo "\$condition_found,\$experiment,\$replicate,\$date_timestamp,\$filename,\$filepath",\$channel_idx,\$glom_idx,\$nuclei_idx >> found_images.txt
            else
...
process QUANTIFY_PODOCYTE_STAINS {
    ...
    output:
    tuple val(metadata), val(image_info), val(unique_id), val(stain_name), path("${stain_name}_MASK_${unique_id}.tif"), path("${stain_name}_${unique_id}.csv"), path("${stain_name}_${unique_id}.h5"), emit: pod_quantify
    
    script:
    def (filename, filepath) = image_info

    """
    python /group/kidn4/ACTIVE/COLOCALISATION_PIPELINE/general_coloc/quantify/quantify_marker_in_nuclei.py \
    -i "${filepath}" \
    -p ${pod_mask} \
    -d ${nuclei_mask} \
...
process PLOT_RESULTS{
    ...
    script:
    """
    python /group/kidn4/ACTIVE/COLOCALISATION_PIPELINE/general_coloc/plotting/plot_in_nuclei.py \
    -i ${mean_csv} \
    -b boxplots \
    -r ridgeplots
...
workflow {
    ...
    csv_files = WRITE_METADATA(QUANTIFY_PODOCYTE_STAINS.out.pod_quantify)
    .collect()
    .map { flat ->
