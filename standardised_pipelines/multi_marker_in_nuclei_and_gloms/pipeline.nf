...
process QUANTIFY_PODOCYTE_STAINS {
    ...
    container params.singularity_image
input:
    tuple val(metadata), val(image_info), val(unique_id), val(nuclei_mask), val(pod_mask), val(stain_name), val(marker_idx)
output:
    tuple val(metadata), val(image_info), val(unique_id), val(stain_name), path("*_MASK_${unique_id}.tif"), path("${stain_name}_${unique_id}.csv"), path("${stain_name}_${unique_id}.h5"), emit: pod_quantify
script:
def (filename, filepath) = image_info
...
workflow {
    ...
    .map { metadata, image_info, unique_id, nuclei_mask,  pod_mask ->
        tuple(metadata, image_info, unique_id, nuclei_mask, pod_mask, stain_value.val, marker_indices.val)
    }
